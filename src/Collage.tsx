...
const paddingAll = (gutter: number, positions: Position[]) =>
  positions.map(({ top, left, width, height }: Position) => ({
    top,
    left,
    width: width - gutter,
...
const mindex = (arr: number[]) =>
  arr.reduce((minIndex, item, i: number) => (item < arr[minIndex] ? i : minIndex), 0);
...
interface Props {
  ...
  renderImage: ({
    ...
    height: number;
    index: number;
  }) => React.ReactElement<{ idx: number }>;
  width: number;
};

const Collage: React.SFC<Props> = (props) => {
  const {
    columns,
    cover,
    gutter,
